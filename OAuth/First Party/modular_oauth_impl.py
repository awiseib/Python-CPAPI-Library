...
# -------------------------------------------------------------------
# Request #2: Using LST to request /md loop
# -------------------------------------------------------------------
def mdInit():
    # Initial, non-computed elements of request to /portfolio/accounts.
    method = 'POST'
    url = f'https://{baseUrl}/md/auth/init'
    oauth_params = {
            "oauth_consumer_key": consumer_key,
            "oauth_nonce": hex(random.getrandbits(128))[2:],
            "oauth_signature_method": "HMAC-SHA256",
...
def mdLoop():
    # Initial, non-computed elements of request to /portfolio/accounts.
    method = 'GET'
    url = f'https://{baseUrl}/md/snapshot?conids=8314@SMART:STK&fields=31,84,86'
    oauth_params = {
            "oauth_consumer_key": consumer_key,
            "oauth_nonce": hex(random.getrandbits(128))[2:],
            "oauth_signature_method": "HMAC-SHA256",
...
def endpointTest():
    # Initial, non-computed elements of request to /portfolio/accounts.
    method = 'GET'
    url = f'https://{baseUrl}/AccountManagement/Balances/XCred'
    oauth_params = {
            "oauth_consumer_key": consumer_key,
            "oauth_nonce": hex(random.getrandbits(128))[2:],
            "oauth_signature_method": "HMAC-SHA256",
