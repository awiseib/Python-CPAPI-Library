...
def confirmReply(jsonContent):
    try:
        if json.loads(jsonContent.content)[0]['id']:
            ...
            method = 'POST'
            url = f'https://api.ibkr.com/v1/api/iserver/reply/{replyId}'
            oauth_params = {
                    "oauth_consumer_key": consumer_key,
                    "oauth_nonce": hex(random.getrandbits(128))[2:],
                    "oauth_signature_method": "HMAC-SHA256",
            ...
            headers = {"Authorization": oauth_header}
            headers["User-Agent"] = "python/3.11"
            accounts_request = requests.Request(method=method, url=url, headers=headers, json=req_data)
            accounts_response = session_object.send(accounts_request.prepare())
            print(pretty_request_response(accounts_response))
            confirmReply(accounts_response)
    except:
        print()
...
# Initial, non-computed elements of request to /portfolio/accounts.
method = 'POST'
url = 'https://api.ibkr.com/v1/api/iserver/account/DU5240685/orders'
oauth_params = {
        "oauth_consumer_key": consumer_key,
        "oauth_nonce": hex(random.getrandbits(128))[2:],
        "oauth_signature_method": "HMAC-SHA256",
